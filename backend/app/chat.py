...
def gerar_aventura_stream(**kwargs):
    ...
    # 1. Geração de Imagem (Usando Pollinations.ai como alternativa gratuita ilimitada)
    if not secoes_customizadas or "gerar_imagem" in secoes_customizadas:
         yield json.dumps({"type": "progress", "message": "Gerando Capa (Pollinations.ai)..."}) + "\n"
         prompt_img = COMMAND_PROMPTS["gerar_imagem"]["prompt"]
         try:
             import urllib.parse
             # O Pollinations gera imagens a partir de um prompt na URL
             safe_prompt = urllib.parse.quote(prompt_img)
             img_url = f"https://image.pollinations.ai/prompt/{safe_prompt}?width=1024&height=512&nologo=true"
             
             yield json.dumps({"type": "data", "section": "gerar_imagem", "content": img_url}) + "\n"
         except Exception as e:
             print(f"Erro na Imagem Alternativa: {e}")
             yield json.dumps({"type": "data", "section": "prompt_imagem_capa", "content": f"Falha ao gerar imagem: {e}"}) + "\n"
